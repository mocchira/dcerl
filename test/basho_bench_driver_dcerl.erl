...
%%
%% -------------------------------------------------------------------
-module(basho_bench_driver_dcerl).
...
new(Id) ->
    case code:which(dcerl) of
        non_existing ->
            io:format("dcerl-benchmark requires dcerl to be available on code path.\n");
        _ ->
            void
    end,
...
run(get, KeyGen, _ValueGen, #state{handler = D, check_integrity = CI} = State) ->
    ...
    case dcerl_api:get(D, BinKey) of
        {ok, D2, Value} ->
            case CI of
                true ->
...
run(put, KeyGen, ValueGen, #state{handler = D, check_integrity = CI} = State) ->
    ...
    Val = ValueGen(),
    BinKey = list_to_binary(Key),
    case dcerl_api:put(D, BinKey, Val) of
        {ok, D2} ->
            case CI of
                true ->
